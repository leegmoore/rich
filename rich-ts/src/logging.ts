...
export class RichHandler {
  ...
  emit(record: LogRecord): void {
    ...
    if (this.richTracebacks && record.exc_info) {
      ...
      if (excType && excValue) {
        if (excValue instanceof Error) {
          traceback = Traceback.fromException(excValue, {
            width: this._tracebacksWidth,
            codeWidth: this._tracebacksCodeWidth,
            extraLines: this._tracebacksExtraLines,
            theme: this._tracebacksTheme ?? undefined,
            wordWrap: this._tracebacksWordWrap,
            showLocals: this._tracebacksShowLocals,
            suppress: this._tracebacksSuppress,
            maxFrames: this._tracebacksMaxFrames,
            localsMaxLength: this._localsMaxLength ?? undefined,
            localsMaxString: this._localsMaxString ?? undefined,
            localsHideDunder: false, // TODO: Add option
            localsHideSunder: false, // TODO: Add option
          });
        } else {
          message = String(excValue);
        }
      }
    }
